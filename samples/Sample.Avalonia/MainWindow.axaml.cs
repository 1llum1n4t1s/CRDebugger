...
public partial class MainWindow : Window
{
    ...
    private void OnAddError(object? sender, RoutedEventArgs e)
    {
        try
        {
            throw new InvalidOperationException("テスト例外です");
        }
        catch (Exception ex)
        {
            CRDebugger.Core.CRDebugger.LogError($"サンプルエラー #{++_logCount}", ex);
        }
    }
}
...
public class SampleOptions
{
    ...
    [CRCategory("Graphics")]
    [CRDisplayName("解像度スケール")]
    [CRRange(0.5, 2.0, Step = 0.1)]
    ...
    [CRCategory("Debug")]
    [CRAction(Label = "GC Collect")]
    public void ForceGC()
    {
        GC.Collect();
        CRDebugger.Core.CRDebugger.Log("GC.Collect() 実行完了");
    }
}
