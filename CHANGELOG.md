...
### Fixed
- Avalonia のデバッガーウィンドウを非表示後に再表示できず、ウィンドウと購読が残り続ける問題を修正
- WinForms の初期化とウィンドウ表示を UI スレッドへ正しく配送し、バックグラウンドスレッドからの操作で例外や更新停止が起きる問題を修正
- コンソールをクリアした直後に処理待ちのログが復活する問題と、レベル別件数が表示上限で不正確になる問題を修正
- 実行中に Options コンテナを追加・削除した際の表示更新を UI スレッドへ配送し、クロススレッド例外を防止
- `BugReportSendTimeout` が送信処理に適用されない問題と、Avalonia でバグレポートのスクリーンショット取得が UI スレッド外から実行される問題を修正
- プロファイラ更新と WinForms のシステムテーマ監視が多重実行され、負荷増加や状態競合を起こしうる問題を修正
